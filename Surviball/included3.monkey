...
Class Ball
	...
	Method collide(game:Tgame)
		Local currenttile:Int = (Int(posy) / TILESIZE) * level.levelWidth + Int(posx) / TILESIZE
...
#End		
		Select (level.level[currenttile])                   
			Case LEVEL_DROP
				' player fell off
		...
		currenttile = Int(posy / TILESIZE) * level.levelWidth + Int(posx / TILESIZE)
		Local xposintile:Float = posx - Int(posx / TILESIZE) * TILESIZE
		Local yposintile:Float = posy - Int(posy / TILESIZE) * TILESIZE
		Local collision:Int = 0
		Local normalx:Float = 0
		Local normaly:Float = 0
		' Check collision with tile edges
		If (level.coins[currenttile] & COLLIDE_E And xposintile > (TILESIZE - RADIUS))
			normalx -= xposintile - (TILESIZE - RADIUS)
			collision = 1
		Endif
		If (level.coins[currenttile] & COLLIDE_W And xposintile < RADIUS)
			normalx += (RADIUS) - xposintile
			collision = 1
		Endif
		If (level.coins[currenttile] & COLLIDE_S And yposintile > (TILESIZE - RADIUS))
			normaly -= yposintile - (TILESIZE - RADIUS)
			collision = 1
		Endif
		If (level.coins[currenttile] & COLLIDE_N And yposintile < RADIUS)
		...
		If (collision)
			...
			xposintile = posx - Int(posx / TILESIZE) * TILESIZE
			yposintile = posy - Int(posy / TILESIZE) * TILESIZE
		Endif
		...
		If (level.coins[currenttile] & COLLIDE_NE And ((nx*nx) + (py*py)) < (RADIUS * RADIUS))
			...
			Endif
		Endif
		If (level.coins[currenttile] & COLLIDE_NW And (px + py) < r2)
			...
			Endif
		Endif
		If (level.coins[currenttile] & COLLIDE_SE And ((nx*nx) + (ny*ny)) < r2)
			...
			Endif
		Endif
		If (level.coins[currenttile] & COLLIDE_SW And ((px*px) + (ny*ny)) < r2)
		...
		If (collision)
			...
			Local dot:Float = movx * normalx + movy * normaly
			
			' Adjust the motion vector based on the collision
			movx -= dot * 1.25 * normalx
			movy -= dot * 1.25 * normaly
			
			' Adjust the roll vector based on the collision
			rollx -= dot * 1.25 * normalx
			rolly -= dot * 1.25 * normaly
		Endif
	...
	Method display(x:Int, y:Int, r:Int,roty:Float, rotz:Float)
		...
		If game.gGameetate <> game.STATE_EXPLODE
		...
		If game.gGameState = game.STATE_INGAME
			If Abs(movx)>.6 Or Abs(movy)>.6
					For Local v:Tvertex = Eachin gRVtxList
						Local p:Tparticle = partStore.get()
						p.init(v.x*(r-dotradius)+posx,v.y*(r-dotradius)+posy,0,0,7)
	...
	Function getKeys()
		ballMoving = False
		If gKeyLeft  rollx -= THRUST ballMoving = True
		If gKeyRight rollx += THRUST ballMoving = True
		If gKeyUp    rolly -= THRUST ballMoving = True
		If gKeyDown  rolly += THRUST ballMoving = True
	End Function
